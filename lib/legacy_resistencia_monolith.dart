...
class Reporte {
  ...
  Reporte({
    ...
    this.lng,
    this.sourcePhone = '',
    List<String>? validadores,
    this.validaciones = 0,
    this.estado = ReporteEstado.pendiente,
    int? duracionMin,
    this.puntosAcreditados = false,
  })  : validadores = List<String>.from(validadores ?? const []),
  ...
  static Reporte fromMap(Map<String, dynamic> m) {
    ...
    return Reporte(
      ...
      sourcePhone: (m['sourcePhone'] as String?) ?? '',
      validadores: ((m['validadores'] as List?) ?? const [])
          .map((e) => e.toString())
          .toList(),
...
class _FirestoreAdminPanelScreenState extends State<FirestoreAdminPanelScreen> {
  ...
  Future<void> _loadConfig() async {
    try {
      ...
      final freeLimit = (cfg['freeLimit'] is int) ? cfg['freeLimit'] as int : 500;
      final priceMonthly = (cfg['priceMonthly'] is int) ? cfg['priceMonthly'] as int : 500;
      final paywallEnabled = (cfg['paywallEnabled'] is bool) ? cfg['paywallEnabled'] as bool : true;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                const Text(
                  'Nota: el Fundador queda fijo y no se puede remover.',
                  style: TextStyle(fontSize: 12),
                ),
              ],
            ),
                        ...
                        if (docs.isNotEmpty) {
                          return Column(
                            children: [
                              ...
                              const SizedBox(height: 8),
                              OutlinedButton.icon(
                                onPressed: () => _openAuxilioDetail(docs.first.reference, myPhoneDigits),
  ...
  Future<void> _pushReporteFS(Reporte r) async {
    try {
      ...
      await FirebaseFirestore.instance.collection(FS_REPORTES_COL).doc(r.id).set({
        ...
        'createdAt': FieldValue.serverTimestamp(),
        'updatedAt': FieldValue.serverTimestamp(),
        'sourcePhone': r.sourcePhone.isNotEmpty ? r.sourcePhone : _onlyDigits(widget.myPhone),
      }, SetOptions(merge: true));
    } catch (_) {}
  }
...
// ================== INICIAR / DETENER ==================
  ...
  void validar(Reporte r) {
    ...
    final me = _onlyDigits(widget.myPhone);
    if (me.isEmpty) {
      snack('⚠ No se pudo identificar tu número para validar.');
      return;
    }

    if (r.sourcePhone.isNotEmpty && r.sourcePhone == me) {
      snack('⚠ No podés validar tu propio reporte.');
      return;
    }

    if (r.validadores.contains(me)) {
      snack('⚠ Ya validaste este operativo.');
      return;
    }
    ...
    r.validadores.add(me);
    r.validaciones = r.validadores.length;
...
class _AuxilioDetailScreenState extends State<AuxilioDetailScreen> {
  ...
  Future<void> _confirmResolved(Map<String, dynamic> d) async {
    ...
    try {
      ...
      final withinBonus = (d['arrivedWithinBonus'] == true);
      final reward = AUX_COMPLETADO_PTS + (withinBonus ? AUX_BONUS_RAPIDEZ_PTS : 0);
      ...
      await FirebaseFirestore.instance.runTransaction((tx) async {
        ...
        tx.update(widget.auxRef, {
          ...
          'rewardPoints': reward,
          'rewardBonus': withinBonus,
        });

        // sumar puntos al ayudante
      ...
      await _appendLog(widget.auxRef, 'confirm', 'Solicitante confirmó. +$reward al ayudante');
      widget.onToast('Confirmado. ¡Gracias!');
    } catch (_) {
      widget.onToast('Error al confirmar.');
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
